...
namespace DataAccessLayer
{
    public class AmazonRepository : IAmazonRepository
    {
        ...
        public  async Task<List<Orderst>> GetAllOrdersByCreatedDate(DateTime dt)
        {
            using (OrdersDbContext dbContext = new OrdersDbContext())
            {
                var amazonOrders3 = await dbContext.Orders.Where(x =>x.CreatedDate > dt).ToListAsync();


                List<Orderst> domainModels = new List<Orderst>();

                foreach (var ord in amazonOrders3)
                {
                    domainModels.Add(new Orderst
                    { 
        ...
        public async Task<List<string>> GetAllUserNames()
        {
            using(OrdersDbContext dbContext = new OrdersDbContext())
            {
                List<string> order1 = await dbContext.Orders.Select(x => x.UserName).ToListAsync();
